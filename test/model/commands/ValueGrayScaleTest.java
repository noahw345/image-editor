...
/**
 * Tests the ValueGrayScale class.
 */
public class ValueGrayScaleTest {
  ...
  public void updatePixel() {
    new ValueGrayScale().goCommand(ex1);
    new ValueGrayScale().goCommand(ex2);
    ...
    for (int r = 0; r < 5; r++) {
      for (int c = 0; c < 5; c++) {
        ...
        assertEquals(ex1.getImage()[r][c].getRGB()[0], Math.max(ex1OG.getImage()[r][c].getRGB()[0],
                Math.max(ex1OG.getImage()[r][c].getRGB()[1], ex1OG.getImage()[r][c].getRGB()[2])));
      }
    }
  }
}